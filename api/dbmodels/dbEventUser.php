...
class dbEventUser
{
    ...
    public static function wxLogin($wxcode, $encryptedData, $iv, $userdata, $gid=1, $fromurid=0, $app=1, $dist=0) {
        ...
        try {
            ...
            //save User
            $isnew = User::findOne(['unionid'=>$unionid]);
            if(empty($isnew)) {
                $isnew = new User();
                $isnew->unionid = $unionid;
    ...
    public static function getMyMemberList($urid,$gid, $page) {
        $query = EventMembers::find()->select('id as memberid,name,mobile,sex,idtype,idnumber,birth,avatar,nation,memberinfos,size,create_time')
            ->andFilterWhere(['urid' => $urid, 'gid' => $gid, 'status' => 1]);
        $limit = 20;
        $total = $query->count();
    ...
    {
        $membermodel = EventMembers::findOne(['id' => $memberid, 'urid' => $urid, 'status' => 1]);
        if (empty($membermodel)) {
            return true;
        }
